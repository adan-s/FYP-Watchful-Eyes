...
import 'package:charts_common/src/common/palette.dart' as charts_common;
...
class _AnalyticsAndReportsState extends State<AnalyticsAndReports> {
  ...
  List<Color> _getColorList(int length) {
    return charts.MaterialPalette.getOrderedPalettes(length)
        .map((palette) => Color.fromRGBO(palette.shadeDefault.r,
            palette.shadeDefault.g, palette.shadeDefault.b, 1))
        .toList();
  }
  ...
  List<Color> _getColorList2(int length) {
    ...
    for (int i = 0; i < length; i++) {
      // Generate a color based on the HSL (Hue, Saturation, Lightness) color model
      double hue = (i / length) * 360; // Varying hue for each color
